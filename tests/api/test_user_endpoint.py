...
def test_create_user(client, app_ctx, db):
    payload = UserFactory.create_payload(login="created_user")
    payload['password'] = 'create_pass'
    del payload['role']
    headers = _auth_header("created_user", "create_pass")
...
def test_get_all_users(client, app_ctx, db):
    for x in range(3):    
    ...
    headers = _auth_header("intuser1", "secret")
...
def test_get_user_by_id(client, app_ctx, db):
    user = UserFactory.build(login="intuser2")
    user.hash_password("secret2")
    db.session.add(user)
    db.session.commit()

    headers = _auth_header("intuser2", "secret2")
