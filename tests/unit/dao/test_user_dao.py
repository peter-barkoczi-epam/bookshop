...
def test_fetch_by_id_returns_user(monkeypatch, app_ctx):
...
def test_fetch_all_returns_list(monkeypatch, app_ctx):
...
def test_delete_fetches_and_deletes_item(monkeypatch, app_ctx):
    user = UserFactory.build(id=1)
